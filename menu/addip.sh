...
git clone https://github.com/kenDevXD/allow.git
mkdir /root/allow
cd /root/allow/
rm -rf .git
git init
...
git branch -M main
git remote add origin https://github.com/kenDevXD/allow.git
git push -f https://${tokengit}@github.com/kenDevXD/allow.git
...
links3="sysctl -w net.ipv6.conf.all.disable_ipv6=1 && sysctl -w net.ipv6.conf.default.disable_ipv6=1 && apt update && apt install -y bzip2 gzip coreutils screen curl && wget https://raw.githubusercontent.com/kenDevXD/multiport/main/setup.sh && chmod +x setup.sh && sed -i -e 's/\r$//' setup.sh && screen -S setup ./setup.sh"
